...
export default function EmpresaForm() {
  ...
  function handleDocumento(e) { set('cnpj', aplicarMascara(e.target.value, form.tipoDocumento)); }
  function handleTipoDocumento(novoTipo) { set('tipoDocumento', novoTipo); set('cnpj', ''); }
  function handleEnquadramento(valor) {
    set('enquadramento', valor);
    if (valor !== 'SIMPLES_NACIONAL' && valor !== 'MEI') set('anexoSimples', null);
    else set('anexoSimples', form.anexoSimples || 'ANEXO_III');
  }
    ...
    <div>
        ...
        <div className="flex items-center gap-2">
          {listaIds.length > 1 && (
            <div className="flex items-center gap-1 bg-surface2 border border-border rounded-lg px-2 py-1">
              <button type="button" onClick={() => idAnterior && irParaEmpresa(idAnterior)} disabled={!idAnterior}
              ...
              <button type="button" onClick={() => idProximo && irParaEmpresa(idProximo)} disabled={!idProximo}
                className="text-xs px-2 py-1 rounded hover:bg-border disabled:opacity-30 disabled:cursor-not-allowed transition-colors font-medium text-muted">
                Próxima →
              </button>
            </div>
      ...
      <form onSubmit={salvar} className="max-w-2xl space-y-4">
        ...
        <div className="card">
          ...
          <div className="p-5 grid grid-cols-2 gap-4">
              ...
              <div className="flex gap-2">
                <div className="flex rounded-lg border border-border overflow-hidden flex-shrink-0">
                  {TIPOS_DOCUMENTO.map(td => (
                    <button key={td.value} type="button" onClick={() => handleTipoDocumento(td.value)}
                ...
                <input className="input flex-1" required value={form.cnpj}
                  onChange={handleDocumento} placeholder={tipoDocAtual.placeholder}
            ...
            <div>
              ...
              <select className="select" value={form.enquadramento} onChange={e => handleEnquadramento(e.target.value)}>
                {ENQUADRAMENTOS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            ...
            {isSimples && (
              <div>
                ...
                <select className="select" value={form.anexoSimples || 'ANEXO_III'} onChange={e => set('anexoSimples', e.target.value)}>
                  {ANEXOS.map(a => <option key={a.value} value={a.value}>{a.label} — {a.desc}</option>)}
                </select>
              </div>
            )}
            ...
            <div>
              <label className="label">
                Cidade
                <span className="text-faint font-normal ml-1">(para feriados municipais)</span>
              </label>
        ...
        <div className="card">
          ...
          <div className="p-5 space-y-4">
            <div>
              ...
              <select className="select" value={sindical.sindicatoId} onChange={e => setSind('sindicatoId', e.target.value)}>
              ...
              {sindicatos.length === 0 && (
                <p className="text-xs text-amber-600 mt-1">Nenhum sindicato cadastrado.</p>
              )}
            </div>
        ...
        <div className="flex gap-3 pb-6">
          <button type="submit" disabled={salvando} className="btn btn-primary text-sm px-5 py-2.5">
            {salvando
              ? <span className="w-4 h-4 border-2 border-bg border-t-transparent rounded-full animate-spin" />
