            ...
            <form onSubmit={salvarSindicato} className="card p-5 mb-4 max-w-2xl">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea className="input h-14 resize-none py-2" value={formSind.observacoes}
                    onChange={e => setFormSind(f => ({ ...f, observacoes: e.target.value }))} />
                </div>

                {/* Vincular empresas — TODAS as empresas */}
                <div>
                  ...
                  {todasEmpresas.length === 0 ? (
                    <p className="text-xs text-faint p-3 bg-surface2 rounded-lg border border-border">Nenhuma empresa cadastrada.</p>
                  ) : (
                      ...
                      {empresasFiltradas.map(emp => (
                        <label key={emp.id}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {emp.temFuncionarios && <p className="text-[10px] text-green-600">com funcionários</p>}
                          </div>
                        </label>
                      ))}
                    </div>
      ...
      {aba === 'cct' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs text-muted">Todas as empresas do escritório.</p>
            {isGestor && (
              <button onClick={() => {
          ...
          {loading ? (
            ...
            <div className="card p-10 text-center">
              <p className="text-muted text-sm">Nenhuma empresa vinculada a sindicato.</p>
            </div>
          ) : (
            <div className="card overflow-hidden">
              <table className="w-full">
