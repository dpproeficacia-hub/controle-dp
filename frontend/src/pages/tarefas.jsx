...
export default function Tarefas() {
  ...
  const filtrados = grupos.filter(g =>
    ...
    g.empresa?.razaoSocial?.toLowerCase().includes(filtroBusca.toLowerCase())
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">
            Gerencie todas as tarefas — por empresa, grupo de empresas ou para todas
          </p>
        </div>
        {isGestor && (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <p className="text-xs text-faint mt-1">Deixe em branco para cobrar desde o cadastro da empresa.</p>
              </div>
            </div>
              ...
              <div>
                ...
                {form.escopo === 'todas' && empresas.length > 0 && (
                  <p className="text-xs text-muted mt-2">
                    A tarefa será criada para as <strong>{empresas.length}</strong> empresas cadastradas.
                  </p>
                )}
              </div>
            )}
                ...
                <div className="space-y-1 mb-2">
                  {form.subtarefas.map((s, i) => (
                    <div key={i} className="flex items-center gap-2 px-3 py-2 bg-surface2 rounded-lg border border-border">
                      ...
                      <button type="button" onClick={() => removerSub(i)}
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <span className="text-xs text-faint">{filtrados.length} tarefa(s)</span>
      </div>
            ...
            <tbody>
              {filtrados.map(g => (
                <tr key={g.id} className="border-b border-border last:border-b-0 hover:bg-surface2">
                  ...
                  <td className="px-4 py-3 text-sm text-muted max-w-[180px] truncate" title={g.empresa?.razaoSocial}>
                  ...
                  <td className="px-4 py-3 text-sm text-muted">
                    {fmtData(g.inicioCobrancaEm) || <span className="text-faint">—</span>}
                  </td>
                  <td className="px-4 py-3 text-sm text-muted">
