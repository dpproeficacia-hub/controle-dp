...
export default function Empresas() {
  ...
  async function excluir(e, emp) {
    e.stopPropagation();
    if (!window.confirm(`EXCLUIR "${emp.razaoSocial}"?`)) return;
        ...
        <div className="card p-5 mb-4 border-2 border-blue-200">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {CAMPOS_LOTE.map(campo => (
              <div key={campo.key} className={`p-3 rounded-lg border transition-all ${camposAtivos[campo.key] ? 'border-blue-300 bg-blue-50' : 'border-border bg-surface2'}`}>
